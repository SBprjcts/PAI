...
run_mls() {
  start bash -c "cd /c/Users/alafs/Desktop/PAI/ML; source venv/Scripts/activate; python app.py; exec bash"
}
...
if [[ "$1" == "-i" ]]; then
  ...
  case "$ftr" in
    ...
    b)
      ...
      ;;
    m)
      echo "[info] Independent run flag provided; MLs argument; running MLs."
      run_mls
      ;;
    *)
      echo "Usage: $0 [-i f|b|m]"
      echo "  -i f   Run frontend only"
      echo "  -i b   Run backend only"
      echo "  -i m   Run MLs only"
      exit 2
      ;;
  esac
  exit 0
